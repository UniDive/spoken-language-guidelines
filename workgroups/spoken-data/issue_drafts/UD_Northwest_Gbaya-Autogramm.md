...
- `sound_url` → document level: the released file has no `# document_id` markers at all, so document boundaries need to be reconstructed first from `sent_id` (format `GYA_..._NNN-NNN`), then `sound_url` can be hoisted:
  1. `python3 workgroups/spoken-data/scripts/harmonize_metadata.py derive-document-id DIR --pattern '^(?P<doc>.+)_[0-9]+[a-z]?[-_][0-9]+[a-z]?$' --write` — dry-run on the real file derives 3 `document_id`s (matching the 3 distinct `sound_url` values and the 3 source files under `not-to-release/`); one irregular `sent_id` (`..._098-098bb`) doesn't match this pattern and needs either a broader regex or a manual fix.
