...
**Finding:** Identifiable via the `document_id` prefix, which already encodes genre.

**Evidence:** `document_id` values are `genre-code`, e.g. `spoken-002R`. Full prefix distribution: news (38), fiction (16), blog (15), nonfiction (9), wiki (7), reviews (3), web (2), spoken (2), social (1).

**Suggestion:** Add `# modality = spoken` to the 2 documents whose `document_id` starts with `spoken-`.
...
| Field       | Suggestion                                                                                                                             |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `doc_title` | `document_id` already exists separately (e.g. `spoken-002R`); `doc_title` is the human-readable document title - keep as corpus-specific |
...
- **Needs a small script:** add `# modality = spoken` to the 2 documents whose `document_id` starts with `spoken-` (`spoken-002R` and one other). Intended one-liner is `harmonize_metadata.py tag-modality UD_Western_Armenian-ArmTDP --spoken-if '^spoken-' --write`, but note the shared script's `COMMENT_RE` currently only matches single-word comment keys (`[A-Za-z_][A-Za-z0-9_]*`), so it silently fails to recognize `# document_id = ...` (a two-word key) - confirmed via dry-run against the local clone (0 matches reported, when there should be 2). Until that regex is widened to allow spaces in the key, use a plain search & replace instead: `sed -i '' '/^# document_id = spoken-/a\
# modality = spoken' *.conllu`.
- **Needs manual input from maintainers:** `translit` - corpus-specific field, needs a naming decision. `doc_title` needs no change (kept as corpus-specific, distinct from `document_id`).
