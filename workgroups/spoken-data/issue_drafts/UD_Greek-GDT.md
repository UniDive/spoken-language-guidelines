...
This treebank mixes spoken and written material but its `.conllu` files don't explicitly mark which sentences are spoken. It's identifiable via the source-outlet component embedded in `newdoc id` (`gdt-<date>-<source>-<docname>`).
...
**Suggestion:** Add `# modality = spoken` to documents whose `newdoc id` source component is `ep`, `ert`, or `ertonline`.
