...
- confirm/apply `# modality = written` for the ~17 biblical `newdoc id` documents and `# modality = spoken` for the ~26 narrative/ritual ones
- clean up 4 leaked Windows file paths used as `newdoc id`, 2 of which appear to duplicate documents that already exist under a clean id
